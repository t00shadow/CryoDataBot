...
class MainWindow(qtw.QMainWindow):    # Make sure the root widget/class is the right type (can check in designer or the .ui file)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.resize(700, 850)        # HOTFIX, functionally same as changing the size in the pyuic5 generated .py file but you aren't meant to edit that (can also just change size in designer, butttt it looks fine in designer). prob should involve screensize or smth, vanilla size is different than qt designer preview. prob cuz of screen resolution and/or dpi settings or smth. high dpi scaling can affect how pixels are rendered
        ...
        # ======== SIGNALS AND SLOTS ========
        # CONSIDER organizing them by page?
        ...
        # buttons
        self.ui.pushButton_6.clicked.connect(self.make_csv)
        self.ui.pushButton_7.clicked.connect(self.gen_dataset_quick)
        self.ui.pushButton_10.clicked.connect(self.gen_dataset)
        self.ui.pushButton_31.clicked.connect(self.browse_folder)


        # custom query TextEdit widget
        self.querywidget = TTEwidget2.TagTextEdit()
        self.ui.B2_queryBox_2.layout().addWidget(self.querywidget)
        self.querywidget.setMaximumHeight(200)
        # flip the names of B2_queryBox and B2_queryBox_2 later (1st page was made from copy of 2nd page so suffixes are +1)
        ...
        # status bar
        self.ui.lineEdit_22.textEdited['QString'].connect(self.ui.statusbar.showMessage)
        # self.ui.lineEdit_22.textEdited.connect(self.ui.statusbar.showMessage)     # works without the ['QString'], look into why
        self.ui.lineEdit_21.textEdited['QString'].connect(self.ui.statusbar.showMessage)
    ...
    # junk fxn for testing signals and slots
    def run_naive(self):
    ...
    def make_csv(self):
        ...
        main_new.search_emdb(processed_query)   # combine the results into a single csv and delete individual csvs or create a folder, should be handled in the util function
    ...
    def gen_dataset_quick(self):
        ...
        main_new.QUERY = " AND ".join(self.querywidget.keywords)       # will likely need a simple parsing fxn here later
        print("query:", main_new.QUERY)
        save_path = self.ui.lineEdit_22.text()
        ...
        if not save_path:        # empty string is "falsy". tho [if self.ui.lineEdit_22.text() == ""] might be more readable
            ...
            main_new.main(save_path, save_path)
    ...
    def browse_folder(self):
        ...
        self.ui.lineEdit_22.setText(self.download_path)
