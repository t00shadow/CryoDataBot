...
def downloading_and_preprocessing(metadata_path, raw_dir: str = 'Raw', overwrite = False):
    ...
    preprocess_maps(csv_info, path_info, metadata_path, give_map=True)
...
def preprocess_one_map(recl: float, raw_map_path: str, model_path: str, give_map: bool=True, protein_tag_dist: int=1, map_threashold=0.15):
    ...
    try:
        ...
        unique, counts = np.unique(protein_tag, return_counts=True)
        print("protein_tag\n", dict(zip(unique, counts)))
        unique, counts = np.unique(map_F, return_counts=True)
        print("map_F\n", dict(zip(unique, counts)))
        print(np.sum(protein_tag), np.sum(map_F))
        print(np.sum(np.logical_and(protein_tag, map_F)))
        print(np.sum(np.logical_or(protein_tag, map_F)))
